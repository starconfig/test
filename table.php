...
function vt_woocommerce_variable_add_to_cart($allsets) {
	...
	if ($product->get_type() != 'variable') {
		...
		return (__('This is not a variable product', 'vartable'));
	}
	...
	foreach ($finalattr as $key => $value) {
      ...
      ';
		...
		if ($vartable_thumb == 1) {
			...
			if (isset($rowimg[0])) {
				$allcolumns['vartable_thumb'] = '
				
				
				<td class="porto-lightbox-container"  data-label="' . apply_filters('vartable_dl_thumb', $headenames['vartable_thumb'], $value) . '">
                 <div class="lightbox" data-plugin-options="{&quot;delegate&quot;:&quot;a&quot;,&quot;type&quot;:&quot;image&quot;,&quot;gallery&quot;:{&quot;enabled&quot;:true}}">
				 <a href="' . $rowimgfull[0] . '" class="porto-popup" data-animation="my-mfp-zoom-in" title="' . $product->get_title() . ' - ' . implode(' - ', $value['attributes']) . '" >
				...
				$allcolumns['vartable_thumb'] = '<td class="porto-lightbox" data-label="' . apply_filters('vartable_dl_thumb', $headenames['vartable_thumb'], $value) . '">
...
?>
		...
		foreach ($attrnames as $attr_slug => $attr_td_value) {
			...
			$allcolumns['vartable_variations'] .= apply_filters('vartable_attributes_join', '<td class="optionscol ' . $attr_slug . '" data-sort-value="' . wc_clean(trim($attr_td_value)) . '" data-label="' . apply_filters('vartable_dl_options', $attr_ordernames[str_replace(array(
				'_',
				'.',
				' '
			) , array(
				...
				'-',
				'-'
			) , sanitize_title(str_replace('attribute_', '', $attr_slug))) ]) . '">' . $attr_td_value . '</td>', $value);
		}
...
function woovartables_scripts() {
	...
	if (get_option('vartable_sorting') == 1) {
		wp_register_script('woovartables_table_sort', plugins_url('assets/js/stupidtable.js', __FILE__) , array(
			'jquery'
		));
	...
	if (get_option('vartable_lightbox') == 1) {
		wp_enqueue_script('vartable_fancybox_js', plugins_url('assets/js/jquery.fancybox.minx.js', __FILE__) , array(
			'jquery'
		));
		wp_enqueue_style('vartable_fancybox_css', plugins_url('assets/css/jquery.fancybox.minx.css', __FILE__));
	}

	wp_register_script('woovartables_js', plugins_url('assets/js/add-to-cart.js', __FILE__) , array(
		'jquery'
	));
